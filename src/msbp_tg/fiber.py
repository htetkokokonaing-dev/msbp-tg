...
def size_bin(series: pd.Series, q: int = 5) -> pd.Series:
    """Quantile size bin with duplicate-edge and all-degenerate handling.

    If all values are identical, pandas.qcut can return all-NaN bins when
    duplicate quantile edges are dropped. For visible-fiber construction,
    that degenerate but valid case is assigned to size bin 0.
    """
